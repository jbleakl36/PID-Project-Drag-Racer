...
[Link To Carolina's Engineering Notebook](https://sites.google.com/charlottesvilleschools.org/engineering2/home?scrlybrkr=4ff37db2)

[Link to Gabe's Engineering Notebook](https://sites.google.com/charlottesvilleschools.org/gabedamiani/not-so-basic-arduino/serial-control)
...
```python  
#Nick Bednar and Joshua Bleakly
#PID Car Cruise Control
#This code calculates rpm using a photointerrupter reading the spokes of a wheel and controls the speed of the car using PID.
...
This is an image of the front of our final product. This specifically shows off our wiring and our ability to fit all components of our project in a fashionable manor on the front of our car.
          
![back right of car](https://github.com/jbleakl36/PID-Project-Drag-Racer/assets/91289646/c42aa870-b285-4e06-bd08-a0c4fe1e182e)
          
This is an image of the back of our final prodcut. This shows off our axle and gear relation and our crafstmanship in creating the main workings of our car.        
...
This is a gif of our car gradually speeding up. This shows that our car functions and all of the work and planning up this point came to fruition in this gif.
          
# Reflection
This project got off to a great start. We immediately began to make plans on a simple PID car and we began making it in CAD. The process went well, we had good communication with each other and did not waste much time. Make sure to have a plan from the very start as you can get a lot done early on. This was our main fault with the project, we failed to have a concrete plan of what we were doing and as a result many missteps took place. Holes weren't added, an encoder wasn't added, minor design faults took place, and the code took vastly more effort than it should've given we were ignorant to the PID library we were given for the first month or so of the project. Nevertheless we both designed different parts for the CAD but we made sure to communicate to each other about what we were making. It's very important to have good communication with your partner, that is something you should at the very least strive for. When it came to the mistakes we made, many of them occured in the CAD stage. We failed to add many hardware holes to your cars base. In order to fix this we opted to make changes and reprint, this time being extremely thoughtful of any other changes we may make in the future as to not have to reprint again. At one point the brackets we made for the motors were too thin and as a result they snapped when any pressure was put on them. To fix this we thickened the brackets and reprinted them. The two front wheels on the car kept slanting left and right so we made coasters that fit on either side of the wheels to keep them straight. One thing we were constantly fighting was the friction that came with our axle. It rubbed against not only the axle holder but also the screws which held the brackets in place. We opted rather than reprinting to instead switch out the screws to smaller ones which didn't rub up against the axle. A large problem we ran into was the fact that one of our DC motors stopped working during testing. We then had to spend lots of time taking apart our car and replacing the motor. This was a great learning experience to make sure to design our parts in a way that you don't have to to take the whole thing apart in order to replace one thing. The main code issue that took place was that our PID file wasn't working properly and as a result the library this project was based in couldn't function. We did some research and found out how to properly install it to get it to work and afterwards code became much easier. Another fault that took place was the prospect of both of us working on CAD. After we had finished all that was left was code and documentation. Nick took on the code which took astronomically longer than any documentation Joshua could have done. This often left Joshua with little to do and Nick left to his own devices. As the clock runs down on the time left to work on this project Nick wishes he had left Joshua to do the CAD and started doing code from the beginning. He also wishes
he had done a little more research about PID through either online sources or past Engineering projects which would've given him a better perspective on how he should approach coding. At this point in time the PID portion of the code is barely functional. It shows small signs of auto correction in an attempt to reach a setpoint but lacks any success.
